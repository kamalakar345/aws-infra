...
resource "aws_eks_cluster" "private_aws_eks" {
  name     = var.eks_name
...
resource "aws_iam_role" "eks_nodes_role" {
  name = "${var.eks_name}_${var.nodename}-role"
  tags = {
		Environment = var.environment
    admin_contact = var.admin_contact
    service_id  = var.service_id
...
resource "aws_iam_policy" "policy" {
  name        = "${var.eks_name}_worker_node_policy"
...
resource "aws_eks_node_group" "node" {
  cluster_name    = aws_eks_cluster.private_aws_eks.name
  node_group_name = "${var.eks_name}_${var.nodename}"
  ...
  remote_access {
    ...
    source_security_group_ids = [aws_security_group.private_keypair_security_group.id]
  }
  tags = {
    key                 = "Name"
    value               = "eks-workers"
  ...
  depends_on = [
    ...
    aws_security_group.private_keypair_security_group,
  ]
}

resource "aws_security_group" "private_keypair_security_group" {
    name        = "dev_private_keypair_new_security_group"
